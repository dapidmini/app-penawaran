document.addEventListener('DOMContentLoaded', function() {
            ...
            .then(data => {
                ...
                data.forEach((item, index) => {
                    ...
                    newRow.addEventListener('click', function() {
                        ...
                        const detailCust = item;
                        console.log('detail pilih cust', detailCust);
                        ...
                        containerDetailPilihCust.forEach((detailElem) => {
                            detailElem.querySelector('#nama').innerHTML = detailCust.nama;
                            detailElem.querySelector('#alamat').innerHTML = detailCust.alamat;
                            detailElem.querySelector('#telepon').innerHTML = detailCust.telepon;
                            detailElem.querySelector('#email').innerHTML = detailCust.email;
                            detailElem.querySelector('#slug').value = detailCust.slug;
                            detailElem.querySelector('#id').value = detailCust.id;
                        });
                    });
    ...
    // ketika mengubah nilai di halaman create di bagian bawah tabel
    const komponenInputFinal = formPenawaran.querySelectorAll('#containerKomponenFinal input');
    komponenInputFinal.forEach(input => {
        console.log('finalinput', input);
        input.addEventListener('blur', function() {
            console.log('blur', input);
            adjustElemenFinalPenawaran(formPenawaran);
        });
    });


    function adjustElemenFinalPenawaran(wrapperElem) {
        let item = {
            totalPenjualanKotorValue: plainNumber(wrapperElem.querySelector('#totalPenjualanKotor').innerHTML),
            totalProfit: plainNumber(wrapperElem.querySelector('#totalProfit').innerHTML)
        };

        let diskonFinal = wrapperElem.querySelector('#diskonFinalInput').value.trim();
        item['diskonFinalOri'] = diskonFinal;
        item['diskonFinalValue'] = cekPersentase(item.diskonFinalOri, item.totalPenjualanKotorValue);

        let biayaFinal = wrapperElem.querySelector('#biayaFinalInput').value.trim();
        item['biayaFinalOri'] = biayaFinal;
        item['biayaFinalValue'] = cekPersentase(item.biayaFinalOri, item.totalPenjualanKotorValue);

        item['totalPenjualanNettValue'] = item.totalPenjualanKotorValue - item.diskonFinalValue + item.biayaFinalValue;

        wrapperElem.querySelector('#diskonFinalValue').innerHTML = currencyFormat(item.diskonFinalValue);
        wrapperElem.querySelector('#biayaFinalValue').innerHTML = currencyFormat(item.biayaFinalValue);
        wrapperElem.querySelector('#totalPenjualanFinal').innerHTML = currencyFormat(item.totalPenjualanNettValue);
    }
    // end function adjustElemenFinalPenawaran
